...
|---|---|---|
| **Universal search** (Phase 1 §4, one search box across goats/medicines/suppliers/invoices) | Deferred during the module-by-module build — the per-page search on the Herd and Inventory lists covers the daily need for a single-user farm; the cross-entity search box was never implemented | Small–medium: `pg_trgm` indexes already exist (Phase 3 §4.3, §7.1); needs one aggregating endpoint + a UI component |
| **Offline-first PWA (IndexedDB outbox, installable app, camera/QR scanning from a phone)** | Phase 1 NFR-04 asked for "offline tolerance," and Phase 2 designed a Workbox/IndexedDB outbox — but the web app as built is a plain React SPA with no service worker, no manifest, no offline queue. It **requires a live connection to save anything** | Medium: add `vite-plugin-pwa`, a manifest, and an outbox layer in `apps/web/src/api.ts` that queues failed mutations and replays them (the backend already supports this via mandatory `Idempotency-Key` — the missing piece is entirely client-side) |
