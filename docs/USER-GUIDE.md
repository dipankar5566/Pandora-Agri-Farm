...
- **The app currently needs a working connection to the farm Wi-Fi/server to save anything** — true offline entry (working with no signal, syncing later) is planned but not yet built; see `docs/Phase-8-Documentation.md` for status. If a save fails, it shows an error rather than silently queuing — try again once connected. **অ্যাপটি এখন সংরক্ষণ করতে খামারের ওয়াইফাই/সার্ভারের সংযোগ লাগে** — সিগন্যাল ছাড়া কাজ করে পরে সিঙ্ক করার সুবিধা এখনো তৈরি হয়নি। সংরক্ষণ ব্যর্থ হলে ত্রুটি দেখাবে, সংযোগ ফিরলে আবার চেষ্টা করুন।
